<template>
  <div class="book-page">
    ...
    <main class="book-list">
      <section
        v-for="(books, categoryName) in filteredBooks"
        :key="categoryName"
      >
        <h2 class="section-title">{{ categoryName }}</h2>
        <div class="book-row">
          <ChooseBookCard v-for="(book, i) in books" :key="i" :book="book" />
        </div>
      </section>
    </main>
...
import ChooseBookCard from "@/components/meeting/ChooseBookCard.vue";
...
const selectedCategoryId = computed(() => {
  return (
    categories.find((c) => c.name === selectedCategoryName.value)?.id || null
  );
});
...
const filteredBooks = computed(() => {
  return {
    [selectedCategoryName.value]:
      bookData.value[selectedCategoryName.value] || [],
  };
});
</script>
